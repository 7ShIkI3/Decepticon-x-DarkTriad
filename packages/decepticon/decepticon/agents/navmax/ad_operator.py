"""NavMAX AD Operator Agent — Active Directory operations via NavMAX tooling.

Wraps ``decepticon.tools.navmax.ad_tools`` into a LangGraph agent
dispatched by the DarkTriad orchestrator.

Library API
-----------
Factory shape mirrors ``langchain.agents.create_agent`` /
``deepagents.create_deep_agent`` — every keyword is optional, and
explicit values fully replace the NavMAX baseline:
...
_SKILL_SOURCES: list[str] = [
    "/skills/navmax/ad_operator/",
    "/skills/shared/",
]

_ROLE = "navmax_ad_operator"
_RECURSION_LIMIT = 1000


def create_navmax_ad_operator_agent(
    *,
    ...
    """Build the NavMAX AD Operator agent.
...
if is_bundle_enabled("navmax"):
    graph = create_navmax_ad_operator_agent()


SUBAGENT_SPEC = SubAgentSpec(
    name="navmax_ad_operator",
    description=(
        "NavMAX Active Directory / Windows attack specialist. "
        "Use after initial internal foothold: LDAP queries, BloodHound "
        "ingestion, Kerberoast / AS-REP roast, ADCS scanning, "
        "and multi-hop AD attack path planning via NavMAX engine."
    ),
    factory=create_navmax_ad_operator_agent,
